...
class HubspotOAuthServiceTest {
    ...
    @Mock
    private CacheServiceGateway cacheServiceGateway;
    @Mock
    ...
    class GenerateAuthorizationUrlTests {
        ...
        void shouldGenerateAuthorizationUrl_WhenUserIsLogged() {
            when(authenticationService.getAuthenticated()).thenReturn(authenticated);
            ...
            verify(authenticationService, times(1)).getAuthenticated();
    ...
    class ProcessTokenExchangeTests {
        ...
        void shouldProcessTokenExchange_WhenStateIsValid() {
            ...
            verify(cacheServiceGateway, times(1))
    ...
    class PersistTokensTests {
        ...
        void shouldNotPersistTokens_WhenTokenResponseIsNull() {
            ...
            verifyNoInteractions(cacheServiceGateway);
        }
    }
    ...
    class GetAccessTokenByUserIdTests {
        ...
        void shouldReturnAccessToken_WhenCachedTokenExists() {
            when(cacheServiceGateway.get("hubspot:access_token:1")).thenReturn("cachedAccessToken");
        ...
        void shouldRefreshAndReturnAccessToken_WhenCachedTokenMissingAndRefreshTokenExists() {
            when(cacheServiceGateway.get("hubspot:access_token:1")).thenReturn(null);
            when(cacheServiceGateway.get("hubspot:refresh_token:1")).thenReturn("testRefreshToken");
            OAuthTokenResponse tokenResponseDTO = new OAuthTokenResponse(
                    "newAccessToken",
                    "newRefreshToken",
                    "Bearer",
            ...
            verify(cacheServiceGateway, times(1)).get("hubspot:access_token:1");
            verify(cacheServiceGateway, times(1)).get("hubspot:refresh_token:1");
            verify(hubspotApiHelper, times(1))
                    .executeCall(anyString(), any(MultiValueMap.class), eq(OAuthTokenResponse.class));
        }
        ...
        void shouldReturnNull_WhenNoAccessOrRefreshTokenFound() {
            when(cacheServiceGateway.get("hubspot:access_token:1")).thenReturn(null);
            when(cacheServiceGateway.get("hubspot:refresh_token:1")).thenReturn(null);
